...
class MainController:
    ...
    def __init__(self, page: ft.Page, sound_dir: Path):
        ...
        # ViewとEditorの紐付け（Viewは後ほどセットされる）
    ...
    def apply_tuning(self, name: str):
        ...
        for item in self.current_tuning_data:
            sound_path = self.sound_dir / item[2] if item[2] else None
    ...
    def on_threshold_change(self, e):
        val = e.control.value
        self.view.threshold_value_text.value = f"振幅閾値: {val:.1f}"
        self.pitch_detector.set_threshold(val)
        self.page.update()

    def on_threshold_change_end(self, e):
        self.config_manager.set_threshold(e.control.value)
    ...
    def play_sound_click(self, e):
        ...
        if not self.view.mode_switch.value:
            ...
            self.view.meter_needle.left = (self.view.meter_width / 2) - 2
    ...
    def _update_ui_callback(self, result_text: str, volume: float, cents: Optional[float]):
        ...
        self.view.volume_bar.value = min(volume * 10.0, 1.0)
        center = self.view.meter_width / 2
        ...
        if cents is not None:
            pos = center + (max(min(cents, 50), -50) * (self.view.meter_width / 100)) - 2
            self.view.meter_needle.left = pos
            self.view.meter_needle.bgcolor = ft.Colors.GREEN_400 if abs(cents) < 5 else ft.Colors.ORANGE_400
        else:
            self.view.meter_needle.left = center - 2
            self.view.meter_needle.bgcolor = ft.Colors.ORANGE_400
        self.page.update()

    def cleanup(self):
        self.is_closing = True
        logging.info("リソースを解放して終了します。")
